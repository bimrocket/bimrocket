...
class FlyTool extends CameraTool
{
  ...
  static ACTIONS = [
    ...
    },
    {
      ...
      value: 1,
      text: "",
      iconName: "base|fly-right"
    },
    {
      name: "options",
      iconName: "base|fly-options"
    }
  ]
  ...
  createPanel()
  {
    ...
    this.panel.bodyElem.classList.add("fly-panel");
    ...
    for (let action of FlyTool.ACTIONS)
    {
      ...
      if (action.iconName)
      {
        Controls.addIcon(button, action.iconName, "icon");
      }
      else
      {
      ...
      I18N.set(button, "title", "base|tool.fly." + action.name);
      I18N.set(button, "alt", "base|tool.fly." + action.name);
      if (action.control)
      {
        let onPressed = event =>
        {
  ...
  onButtonClick(action)
  {
    if (action.name === "options")
    {
      ...
      const dialog = new Dialog("base|tool.fly.options");
      dialog.setClassName("fly-options");
      ...
      const stickControlElem = dialog.addCheckBoxField("stick",
        "base|tool.fly.stick_control", this.mode === "stick");

      const detectCollisionElem = dialog.addCheckBoxField("collision",
        "base|tool.fly.detect_collision", this.detectCollision);

      const groundDistanceControlElem = dialog.addCheckBoxField(
        "ground_control", "base|tool.fly.ground_distance_control",
        this.groundDistanceControlEnabled);

      const groundDistanceElem = dialog.addNumberField("ground",
        "base|tool.fly.ground_distance", this.groundDistance, "ground");
