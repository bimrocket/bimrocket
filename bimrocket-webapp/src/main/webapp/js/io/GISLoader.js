BIMROCKET.GISLoader = class extends THREE.Loader
{
  ...
  load(url, onLoad, onProgress, onError)
  {
    ...
    request.onreadystatechange = function()
    {
      if (request.readyState === 4)
      {
        ...
        {
          var group = scope.parse(request.responseXML ?
            request.responseXML : request.responseText);

          if (scope.modelCount > 0)
          {
            scope.loadModels(group, onLoad);
          }
          else
          {
            onLoad(group);
  ...
  createPoint(name, coordinates, properties, parent)
  {
    ...
    if (modelURL) // deferred point creation
    {
      ...
      {
        modelLoader = new BIMROCKET.GISModelLoader(modelURL);
        this.modelLoaders[modelURL] = modelLoader;
        this.modelCount++;
      }
      const scope = this;
      modelLoader.listeners.push(function(model)
      {
        ...
        scope.setObjectProperties(object, name, properties);
        parent.add(object);
      });
    }
  ...
  createLineString(name, coordinates, properties, parent)
  {
    ...
    else
    {
      ...
      {
        console.warn("BIMROCKET.GisLoader: " + ex);
      }
    }
    if (line)
    {
  ...
  createTube(coordinates, diameter, sides, offset, material)
  {
    ...
    return new BIMROCKET.Solid(geometry, material);
  }
};

BIMROCKET.GISModelLoader = class extends THREE.Loader
{
  ...
  load(onModelLoad)
  {
    ...
    loader.load(this.modelURL, function(model)
    {
      for (let i = 0; i < scope.listeners.length; i++)
      {
        let listener = scope.listeners[i];
