...
public class IfcJavascriptClassesGenerator
{
  ...
  private String packageName = "IFC";
  private String schemaVersion = "IFC2x3";
  private String baseEntityCreator = "createBaseEntity";
  ...
  public String getBaseEntityCreator()
  {
    return baseEntityCreator;
  }

  public void setBaseEntityCreator(String baseEntityCreator)
  {
  ...
  protected void writeHeader(String schemaFileName)
  {
    ...
    writer.println(packageName + " = typeof " + packageName + 
      " === 'undefined' ? {} : " + packageName + ";");
    writer.println("let IFC = " + packageName + "." + schemaVersion + " = {};");
    writer.println();
    
    writer.println("IFC.IfcBaseEntity = " + baseEntityCreator + "(IFC);");
    writer.println();
  }

  protected void writeFooter()
  {
    writer.println("const ifcClassNames = Object.getOwnPropertyNames(IFC);");
    writer.println("for (var i = 0; i < ifcClassNames.length; i++)");
    writer.println("{");
    writer.println("  let ifcClassName = ifcClassNames[i];");
    writer.println("  IFC[ifcClassName.toUpperCase()] = IFC[ifcClassName];");
    writer.println("}");
    writer.println();
  ...
  protected void writeEntity(ExpressEntity entity)
  {
    ...
    // write this entity
    writer.print("IFC." + entity.getName());
    writer.print(" = class");
    writer.println(" extends IFC." + (superEntity == null ? 
      "IfcBaseEntity" : superEntity.getName()));
    writer.println("{");
    ...
    if (!attributes.isEmpty())
    {
      ...
      if (attributeCount > 0) writer.println();
    }
    writer.println("  static ifcClassName = \"" + entity.getName() + "\";");
    writer.println("  static ifcSuperClassName = " +  (superEntity == null ? 
      "null" : "\"" + superEntity.getName() + "\"") + ";");
    writer.println("};");
    writer.println();
  ...
  protected void writeDefinedType(ExpressDefinedType type)
  {
    ...
    writer.println("IFC." + type.getName() + " = class");
    writer.println("{");
    writer.println("  Value = null; // " + definition);  
    writer.println();
    writer.println("  static ifcClassName = \"" + type.getName() + "\";");
    writer.println("};");
    writer.println();
  }
  ...
  public static void main(String[] args) throws IOException
  {
    if (args.length < 2)
    {
      System.out.println("Arguments: schemaFile outputFile " + 
        "[-p:packageName] [-v:schemaVersion] [-b:baseEntityCreator] ");
    }
    else
    {
      ...
      for (String arg : args)
      {
        ...
        {
          generator.setBaseEntityCreator(arg.substring(3));          
        }
