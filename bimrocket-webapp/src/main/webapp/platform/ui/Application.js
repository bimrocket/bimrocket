...
import { Panel, PanelManager } from "platform/ui/panel/Panel.js";
import { ProgressBar } from "platform/ui/progressbar/ProgressBar.js";
import { MenuBar } from "platform/ui/menu/Menu.js";
import { ToolBar } from "platform/ui/toolbar/ToolBar.js";
...
import { Setup } from "platform/utils/Setup.js";
import { ObjectUtils } from "platform/utils/ObjectUtils.js";
import { WebUtils } from "platform/utils/WebUtils.js";
import { PointSelector } from "platform/utils/PointSelector.js";
import { ObjectBatcher } from "platform/utils/ObjectBatcher.js";

import { LineDashedShaderMaterial } from "platform/materials/LineDashedShaderMaterial.js";

import { CSS2DRenderer } from "platform/renderers/CSS2DRenderer.js";
import { SVGRenderer } from "platform/renderers/SVGRenderer.js";

import { Formula } from "platform/formula/Formula.js";

import { EffectComposer } from "platform/postprocessing/EffectComposer.js";
import { RenderPass } from "platform/postprocessing/RenderPass.js";
import { SAOPass } from "platform/postprocessing/SAOPass.js";
import { OutputPass } from "platform/postprocessing/OutputPass.js";
...
import WebGL from "platform/utils/WebGL.js";

import { Environment } from "environment";
import * as THREE from "three";

CSSManager.setTheme("quartz", "../assets/themes/quartz.css", import.meta);
CSSManager.setTheme("carbon", "../assets/themes/carbon.css", import.meta);
...
class Application
{
  ...
  constructor(element = document.body)
  {
    ...
    const bigLogoImage = logoPanelElem.querySelector("svg");
    ...
    IconManager.setSprite("platform", "platform/assets/icons/misc.svg");
    Controls.addIcon(logoButton, "bimrocket", "logo");
    ...
    this.progressBar.setI18N(this.i18n);
    ...
    this.toolBar.position = setup.toolBarPosition;
  ...
  isCanvasEvent(event)
  {
    ...
    {
      if (elem.nodeName === "HEADER" ||
          elem.classList.contains("toolbar") ||
          elem.classList.contains("panel") ||
          elem.classList.contains("panel-expander") ||
          elem.classList.contains("h-resizer") ||
          elem.classList.contains("v-resizer")) return false;
  ...
  useTool(tool)
  {
    ...
    {
      if (this.tool === tool) // use the active tool again, reactivate it
      {
        if (tool)
        {
          this.tool.reactivate();
          toolEvent = { type : "reactivated", tool : this.tool };
          this.notifyEventListeners("tool", toolEvent);
        }
        return;
      }

      if (this.tool) // a tool is active, deactivate it
      {
      ...
      if (tool) // activate the new tool
      {
        tool.activate();
        toolEvent = { type : "activated", tool : tool };
        this.notifyEventListeners("tool", toolEvent);
      }
      else // no tool is active
      {
  ...
  createToolPanel(tool)
  {
    const panel = new Panel(this);

    panel.id = tool.name;
    panel.title = tool.label;
    panel.iconName = tool.getIconName();
    panel.tool = tool;
  ...
  {
    // module activation
    await ModuleManager.activateModules(this);

    // Bundle loading
    BundleManager.setBundle("platform", "platform/i18n/platform");
    this.i18n.defaultBundle = BundleManager.getBundle("platform");
    this.i18n.addSupportedLanguages("en", "es", "ca");
    await this.i18n.updateTree(this.element);

    // Icon loading
    await IconManager.updateTree(this.element);

    // CSS loading
    if (!this.setup.theme) this.setup.theme = "quartz";
    CSSManager.setCurrentTheme(this.setup.theme);
    await CSSManager.update();
  }
...
window.$url = (path, importMeta) =>
{
  return new URL(path, importMeta.url).href;
};
