...
class Setup
{
  ...
  static THEME = "theme";
  static TOOLBAR_POSITION = "toolBarPosition";
  ...
  set theme(name)
  {
    this.setItem(Setup.THEME, name);
    CSSManager.setCurrentTheme(name);
    CSSManager.update();
  }

  get theme()
  {
    return this.getItem(Setup.THEME) || null;
  }

  set toolBarPosition(position)
  {
    this.setItem(Setup.TOOLBAR_POSITION, position);
    this.application.toolBar.position = position;
    this.application.panelManager.updateLayout();
  }

  get toolBarPosition()
  {
    return this.getItem(Setup.TOOLBAR_POSITION) || "bottom";
  }

  applyBackground()
  {
    document.documentElement.style.setProperty(
      "--background-color-1", this.backgroundColor1);

    document.documentElement.style.setProperty(
      "--background-color-2", this.backgroundColor2);
  }
